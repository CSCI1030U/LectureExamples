...
def prob_of_4_of_5_tails(num_iterations):
    num_wins = 0
    for iteration in range(num_iterations):
        flip_vals = np.random.random(5)
        # flip = map(is_tails, flip_vals)
        flip = map(lambda prob : prob < 0.5, flip_vals)
        if (list(flip)).count(True) >= 4:
            num_wins += 1

    return num_wins / num_iterations

print(f'{prob_of_4_of_5_tails(10000) = }')

# exercise 10.2 (estimate prob of 2/5 sixes on d6)
def to_die_roll(prob):
    if prob < 1/6:
        return 1
    elif prob < 2/6:
        return 2
